...
require('tap')(function (test)
  ...
  test("no command results in default being used", function()

    local ran = false
    cli.addCommand("def",function(params,flags)
      assert(params[1] == "param","first parameter is wrong")
      assert(params[2] == "param2","second parameter is wrong")
      assert(flags[1] == "flag","first flag is wrong")
      assert(flags[2] == "flag2","second flag is wrong")
    ...
    cli.setDefault("def")
    cli.run({"-flag","param","-flag2","param2"})
    assert(ran,"the default function was not run")
  end)
end)
